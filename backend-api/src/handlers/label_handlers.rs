...
pub async fn create_label_handler(
    ...
    let created_label = web::block(move || {
    ...
    .await
    .map_err(|e| {
        log::error!("Blocking task error (create_label): {:?}", e);
        ServiceError::InternalServerError("Error processing create_label request".to_string())
    })??;
...
pub async fn list_labels_handler(
    ...
    .await
    .map_err(|e| {
        log::error!("Blocking task error (list_labels): {:?}", e);
        ServiceError::InternalServerError("Error processing list_labels request".to_string())
    })??;
...
pub async fn get_label_handler(
    ...
    let label_option = web::block(move || {
        let mut conn = pool.get()?;
        labels
            .filter(user_id.eq(user_uuid))
    ...
    .await
    .map_err(|e| {
        log::error!("Blocking task error (get_label): {:?}", e);
        ServiceError::InternalServerError("Error processing get_label request".to_string())
    })??;
...
pub async fn update_label_handler(
    ...
    let updated_label = web::block(move || {
        let mut conn = pool.get()?;
        diesel::update(
            labels
    ...
    .await
    .map_err(|e| {
        log::error!("Blocking task error (update_label): {:?}", e);
        ServiceError::InternalServerError("Error processing update_label request".to_string())
    })??;
...
pub async fn delete_label_handler(
    ...
    .await
    .map_err(|e| {
        log::error!("Blocking task error (delete_label): {:?}", e);
        ServiceError::InternalServerError("Error processing delete_label request".to_string())
    })??;
